...
BASE_URL_CRM = 'https://crm.gnpsa.co'
...
class HandleAgendamiento(handlepincancelar):
	...
	def reprogramar_ordenes(self, ordenes, regional):
		
		"""
		Envía las órdenes y espera el resultado haciendo polling.
		Retorna el job completo cuando termina, o None si falla/timeout.
		"""
		intervalo_polling=5	
		timeout=3600

		# — Paso 1: Lanzar el job —
		resp = requests.get(f"{BASE_URL_CRM}/reprogramar_modulo", json={
			"ordenes": ordenes,
			"regional": regional
		})

		if resp.status_code != 202:
			print(f"Error al lanzar job: {resp.status_code}")
			return None

		job_id = resp.json()["job_id"]
		print(f"Job lanzado → {job_id}")

		# — Paso 2: Polling hasta completar o timeout —
		tiempo_transcurrido = 0

		while tiempo_transcurrido < timeout:
			time.sleep(intervalo_polling)
			tiempo_transcurrido += intervalo_polling

			status_resp = requests.get(f"{BASE_URL_CRM}/job_status/{job_id}")
			job = status_resp.json()
			status = job.get("status")

			print(f"[{tiempo_transcurrido}s] Job {job_id} → {status}")

			if status == "completed":
				print(f"✓ Completado en ~{tiempo_transcurrido}s")
				return job

			elif status == "failed":
				print(f"✗ Falló: {job.get('error')}")
				return job

			# Si sigue en queued/running, continúa el loop
