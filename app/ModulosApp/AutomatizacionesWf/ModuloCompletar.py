...
def iniciar_ruta(self,driver):		
	...
	time.sleep(1)
	if "Activar ruta" in driver.find_element(by=By.XPATH, value='/html/body/div[25]').text:
		driver.find_element(by=By.XPATH, value='//*[@aria-label="Activar ruta"]').click()
		time.sleep(1)
		driver.find_element(by=By.XPATH, value='//*[@class="button submit" and @type="submit" and contains(text(),"OK")]').click()
...
def configurar_busqueda(self,driver):
	print("buscando...")
	driver.find_element(by=By.XPATH, value='//*[@class="toa-header-search"]').click()
	...
	for i in range(1,36):
		x=driver.find_element(by=By.XPATH, value='/html/body/div[15]/div[1]/div[2]/div[2]/div/div/div[2]/ul/li['+str(i)+']/div/div[2]/input')
		y=x.get_property('checked')
		#print("*"*40, y ,"*"*40)
		if i==24 and y==False:
			driver.find_element(by=By.XPATH, value='/html/body/div[15]/div[1]/div[2]/div[2]/div/div/div[2]/ul/li['+str(i)+']/div/div[2]/input').click()
		elif i!=24 and y==True:
			driver.find_element(by=By.XPATH, value='/html/body/div[15]/div[1]/div[2]/div[2]/div/div/div[2]/ul/li['+str(i)+']/div/div[2]/input').click()
	'''for i in driver.find_elements(by=By.XPATH, value='//*[@class="toa-search-preferences-checkbox"]'):
		y=i.get_attribute('checked')
		if y!="checked":
			driver.find_element(by=By.XPATH, value='//*[@type="checkbox"]').click()
		else:
			print("estado check ok!")'''
		
	driver.find_element(by=By.XPATH, value='/html/body/div[15]/div[1]/div[2]/div[1]/button').click()
	driver.find_element(by=By.XPATH, value='//*[@class="toa-header-search"]').click()
	try:
...
def cambiar_vista(self,driver):
	driver.find_element(by=By.XPATH, value='//*[@name="popup" and @data-ctrl-id="88"]').click()
	en=driver.find_element(by=By.XPATH, value='//*[@type="range"]')
	move = ActionChains(driver)
	move.click_and_hold(en).move_by_offset(12, 0).release().perform()
	driver.find_element(by=By.XPATH, value='//*[@value="apply"]').click()
...
def selector_Completacion(self,idBot,Idactividad):
		...
		try:
			...
			ArrayOtsCom=[]
			#print(len(array_datos),array_datos)
